...
import { BNBXAdvancedTokenDeployerV4 } from "./BNBXAdvancedTokenDeployerV4.sol";
...
/// @title BNBX V4 dividend launch factory
...
contract BNBXRewardsFactoryV4 {
    ...
    BNBXAdvancedTokenDeployerV4 public immutable tokenDeployer;
    ...
    constructor(
        ...
        if (BNBXAdvancedTokenDeployerV4(tokenDeployer_).manager() != address(0)) {
            revert InvalidAddress();
        }
        feeRecipient = feeRecipient_;
        pancakeV2Router = pancakeV2Router_;
        tokenDeployer = BNBXAdvancedTokenDeployerV4(tokenDeployer_);
    }
    ...
    function _deployToken(
        ...
        address marketing
    ) internal returns (BNBXDividendTokenV4 token) {
    ...
    function _validateMinimumRewardShare(CreateRequest memory request)
        internal
        pure
    {
        if (
            request.minimumRewardShare == 0
                || (request.template == TemplateConfigV4.Template.HolderRewards
                    && request.minimumRewardShare <= MIN_HOLDER_REWARD_SHARE)
