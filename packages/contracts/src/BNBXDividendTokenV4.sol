...
import { BNBXRewardVaultV4 } from "./BNBXRewardVaultV4.sol";
import { TemplateConfigV4 } from "./libraries/TemplateConfigV4.sol";

/// @title BNBX V4 immutable holder/LP dividend token
...
contract BNBXDividendTokenV4 {
    struct Init {
        ...
        address rewardToken;
        TemplateConfigV4.Taxes taxes;
        TemplateConfigV4.Template template;
    ...
    uint256 public constant MIN_HOLDER_REWARD_SHARE = 1_000 ether;
    ...
    TemplateConfigV4.Template public immutable template;
    uint256 public immutable minimumRewardShare;
    BNBXRewardVaultV4 public immutable rewardVault;
    TemplateConfigV4.SideTaxes public buyTaxes;
    TemplateConfigV4.SideTaxes public sellTaxes;
    ...
    error Reentrancy();
    error InvalidMinimumRewardShare();
    ...
    constructor(Init memory init) {
        ...
        TemplateConfigV4.validate(init.taxes);
        if (
            init.minimumRewardShare == 0
                || (init.template == TemplateConfigV4.Template.HolderRewards
                    && init.minimumRewardShare <= MIN_HOLDER_REWARD_SHARE)
        ) revert InvalidMinimumRewardShare();
        ...
        vault.setExcluded(address(configuredRouter), true);
        ...
        external
    {
        ...
        if (template == TemplateConfigV4.Template.LPRewards) {
    ...
    function unlockLiquidityPair() external {
        ...
        emit TaxesActivated(
            uint16(TemplateConfigV4.total(buyTaxes)),
            uint16(TemplateConfigV4.total(sellTaxes))
        );
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        TemplateConfigV4.SideTaxes memory taxes =
    ...
    function _syncHolderShare(address account) internal {
        if (
            template != TemplateConfigV4.Template.HolderRewards
                || isTaxExempt[account]
        ) return;
