...
import { BNBXHolderRewardsTokenDeployer } from "./BNBXHolderRewardsTokenDeployer.sol";
...
/// @title BNBX independent holder-rewards V2 launch Factory
contract BNBXHolderRewardsFactory {
    struct CreateRequest {
        ...
        bytes32 vanitySalt;
        address rewardToken;
        BNBXHolderRewardsToken.Taxes taxes;
    ...
    BNBXHolderRewardsTokenDeployer public immutable tokenDeployer;
    ...
    event HolderRewardsConfigured(
        address indexed token,
        address indexed rewardToken,
        BNBXHolderRewardsToken.SideTaxes buyTaxes,
        BNBXHolderRewardsToken.SideTaxes sellTaxes,
        uint256 minimumRewardBalance
    );
    ...
    constructor(address feeRecipient_, address router_, address defaultRewardToken_) {
        ...
        tokenDeployer = new BNBXHolderRewardsTokenDeployer();
    }
    ...
    function findVanitySalt(
        ...
        BNBXHolderRewardsToken.Init memory init = _tokenInit(request);
        ...
        returns (BNBXHolderRewardsToken.Init memory)
    {
        ...
        return BNBXHolderRewardsToken.Init({
            ...
            taxes: request.taxes,
            minimumRewardBalance: request.minimumRewardBalance
        });
    }
    ...
    function _validateRewardToken(address rewardToken_) internal view {
        ...
        address wbnb = router.WETH();
        if (
            rewardToken_ == address(0) || rewardToken_ == wbnb
