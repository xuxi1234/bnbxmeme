...
import { BNBXZeroTaxToken } from "./BNBXZeroTaxToken.sol";
...
/// @title BNBX zero-tax launch Factory
/// @notice Creates only immutable, fixed-supply BNBXZeroTaxToken launches and
/// routes their internal bonding-curve trades.
contract BNBXZeroTaxFactory {
    ...
    constructor(address feeRecipient_, address pancakeV2Router_) {
        if (
            feeRecipient_ == address(0) || feeRecipient_ == DEAD
                || pancakeV2Router_ == address(0)
                || pancakeV2Router_ == DEAD
    ...
    {
        if (msg.value <= CREATION_FEE) revert InvalidCreationValue();
        if (buyRequest.refundRecipient == address(0)) {
            revert InvalidAddress();
        }
    ...
    function sell(
        ...
        uint256 minBNBOut,
        uint256 deadline
    ) external nonReentrant returns (uint256 netBNB) {
    ...
    function _deployVanityToken(
        ...
        bytes32 vanitySalt
    ) internal returns (BNBXZeroTaxToken token) {
        ...
        token = new BNBXZeroTaxToken{ salt: vanitySalt }(
    ...
    function _deployCurve(
        ...
        IPancakeV2Factory pancakeFactory =
            IPancakeV2Factory(router.factory());
    ...
    /// @notice Searches a bounded salt range through eth_call. This function
    /// cannot spend BNB or mutate contract state.
