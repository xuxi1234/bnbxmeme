...
import { BNBXHolderRewardsVault } from "./BNBXHolderRewardsVault.sol";

/// @title BNBX immutable Holder Rewards V2 token
/// @notice Fixed liquidity, rewards, and burn taxes with no owner or setters.
contract BNBXHolderRewardsToken {
    ...
    uint256 public constant MIN_HOLDER_REWARD_BALANCE = 1_000 ether;
    ...
    BNBXHolderRewardsVault public immutable rewardVault;
    ...
    constructor(Init memory init) {
        ...
        if (
            init.minimumRewardBalance <= MIN_HOLDER_REWARD_BALANCE
                || init.minimumRewardBalance > TOTAL_SUPPLY
        ...
        if (
            init.rewardToken == configuredWbnb
                || !_hasRewardLiquidity(configuredRouter, init.rewardToken, configuredWbnb)
        ...
        BNBXHolderRewardsVault vault = new BNBXHolderRewardsVault(
            address(this),
            init.rewardToken,
            init.minimumRewardBalance
        );
        rewardVault = vault;
        ...
        _syncHolderShare(init.launchManager);
    ...
    function configureLaunch(address curve, address pair) external {
        ...
        if (
            curve == address(0) || curve == DEAD || pair == address(0)
                || pair == DEAD || curve == pair || curve.code.length == 0
                || pair.code.length == 0
        ) revert InvalidAddress();
        ...
        rewardVault.setExcluded(curve, true);
        rewardVault.setExcluded(pair, true);
        launchManager = DEAD;
    ...
    function rewardShare(address account) external view returns (uint256) {
        return rewardVault.shares(account);
    }

    function totalRewardShares() external view returns (uint256) {
        return rewardVault.totalShares();
    }

    function totalRewardsDistributed() external view returns (uint256) {
        return rewardVault.totalRewardsReceived();
    }
