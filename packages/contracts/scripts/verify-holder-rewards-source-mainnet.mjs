...
if (verifyLaunchedTokens) {
  ...
  if (tokenCount > maxTokens) {
    throw new Error(
      `Holder Rewards Factory has ${tokenCount} tokens; raise BNBX_VERIFY_MAX_TOKENS to verify all`,
    );
  }
  ...
  for (let index = 0n; index < tokenCount; index += 1n) {
    ...
    await Promise.all([
      requireCode(token, "Holder Rewards token"),
    ...
    await verify(
      token,
      "src/BNBXHolderRewardsToken.sol:BNBXHolderRewardsToken",
      encodeAbiParameters(
        [
          {
            type: "tuple",
