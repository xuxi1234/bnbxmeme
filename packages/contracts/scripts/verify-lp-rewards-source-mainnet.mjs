...
import { createLPRewardsVerificationInputs } from "./verification-compiler-input.mjs";
...
const factoryInput = process.env.BNBX_LP_REWARDS_FACTORY_ADDRESS;
const verifyLaunchedTokens = process.env.VERIFY_LAUNCHED_TOKENS !== "0";
const root = resolve(import.meta.dirname, "..");

const verificationInputs = createLPRewardsVerificationInputs(root);
...
if (!factoryInput)
  throw new Error("BNBX_LP_REWARDS_FACTORY_ADDRESS is required");
...
async function verify(
  ...
  for (let attempt = 1; attempt <= 30; attempt += 1) {
    ...
    const status = await callApi({ action: "checkverifystatus", guid: result });
...
await requireCode(factory, "LP Rewards Factory");
...
await requireCode(tokenDeployer, "LP Rewards Token Deployer");

await verify(
  factory,
  "src/BNBXLPRewardsFactory.sol:BNBXLPRewardsFactory",
  encodeAbiParameters(
...
await verify(
  tokenDeployer,
  "src/BNBXLPRewardsTokenDeployer.sol:BNBXLPRewardsTokenDeployer",
  "0x",
  verificationInputs.tokenDeployer,
  "lp-rewards-token-deployer",
);
...
if (verifyLaunchedTokens) {
  ...
  if (tokenCount > maxTokens) {
    throw new Error(
      `LP Rewards Factory has ${tokenCount} tokens; raise BNBX_VERIFY_MAX_TOKENS to verify all`,
    );
  }
  ...
  for (let index = 0n; index < tokenCount; index += 1n) {
    ...
    await Promise.all([
      requireCode(token, "LP Rewards token"),
    ...
    const [
      ...
      readValue(
        token,
        uintReadAbi("minimumWbnbValue"),
        "minimumWbnbValue",
      ),
      readAddress(token, "rewardVault"),
      readAddress(token, "liquidityPair"),
      readAddress(token, "wbnb"),
      readAddress(token, "launchCurve"),
    ]);

    requireAddress(launchCurve, curve, "Token launch Curve");

    await requireCode(rewardVault, "LP Rewards Vault");

    await verify(
      token,
      "src/BNBXLPRewardsToken.sol:BNBXLPRewardsToken",
      encodeAbiParameters(
        [
          {
            type: "tuple",
            components: [
              ...
              { name: "minimumWbnbValue", type: "uint256" },
            ],
          },
        ],
        [
    ...
    await verify(
      rewardVault,
      "src/BNBXLPRewardsVault.sol:BNBXLPRewardsVault",
      encodeAbiParameters(
        [
          { type: "address" },
          { type: "address" },
    ...
    const [curveFee, creator, curvePair, curveWbnb, graduationTarget] =
      await Promise.all(
      [
